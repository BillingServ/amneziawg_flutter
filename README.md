...
| ----------- | ------- | ----- | ----- | ------- | ----- |
| **Version** | SDK 24+ | 15.0+ | 12+   | 7+      | Any   |

AmneziaWG is supported on Android, iOS, macOS, and Windows in this fork.


### Windows

On Windows, the app must be run as administrator to be able to create and manipulate the tunnel. The current integration uses `amneziawg-windows` `tunnel.dll` artifacts and launches them through the embedded `/service` runner flow.

The repository currently includes Windows tunnel DLLs under `windows/x64` and `windows/x86`. The next step is runtime validation on a Windows machine.
